"""Phase 7 e20 — Asia R:R improvement + window variants.

Hypothesis: e19 confirmed mean-rev is fragile to filters.
Real bottleneck = R:R asymmetric (small TP, bigger SL).

Variants:
  e20a — Tighter SL: SL at box edge ± buffer (not breakout extreme)
  e20b — Opposite-edge TP: full reversion target (max win per trade)
  e20c — Non-adaptive max_attempts=3 (test if adaptive over-restricts)
  e20d — Late-Asia window 21:00-23:00 ET (1260-1380)
  e20e — Combo: tighter SL + opposite TP
"""
...
from run_phase7_e19_asia_push import filter_allow_all


def backtest_meanrev_rr(date_groups, all_dates, bh, bm, dur, variant,
                         allowed_fn=None, session_name="Asia", adaptive=True,
                         sl_mode="extreme",          # "extreme" | "box_edge"
                         tp_mode="box_mid",          # "box_mid" | "opposite_edge"
                         max_attempts_override=None):
    """Mean-rev with R:R variations.

    sl_mode="box_edge" → SL at box edge + buffer (much tighter than extreme + buffer)
    tp_mode="opposite_edge" → TP at opposite box edge (max reversion)
    """
    ...
    for day in all_dates:
        ...
        for i in range(len(Cl)):
            ...
            if pending is not None and not in_trade:
                ...
                # SL placement
                if sl_mode == "extreme":
                    if ed == 1: sp = extreme - SL_BUFFER
                    else: sp = extreme + SL_BUFFER
                elif sl_mode == "box_edge":
                    # SL just beyond box edge (much tighter, more SL hits but smaller losses)
                    if ed == 1: sp = bx_lo - SL_BUFFER
                    else: sp = bx_hi + SL_BUFFER
                else:
                    sp = extreme  # fallback
                ...
                if tp_mode == "box_mid":
                    ...
                    tp_ = bx_lo if ed == 1 else bx_hi  # full reversion
                else:
                    tp_ = box_mid
...
def walk_forward_e20(df, asia_window, asia_kwargs, london_cfg, ny_cfg):
    ...
    sess_models = {"Asia": "mean_rev_rr",
                   "London": london_cfg["model_type"], "NY": ny_cfg["model_type"]}
...
def run_variant(label, df, asia_window=None, asia_kwargs=None):
    cfg = base_config()
    cfg["NY"]["model_type"] = "direct_breakout"
    cfg["NY"]["pattern_at_breakout"] = "any"
    if asia_window is None:
        asia_window = (CONFIG['sessions']['Asia']['start_min'], CONFIG['sessions']['Asia']['end_min'])
    if asia_kwargs is None:
        asia_kwargs = {"sl_mode": "extreme", "tp_mode": "box_mid"}

    print(f"\n{'═' * 72}\n {label}\n Window: {asia_window}, kwargs: {asia_kwargs}\n{'═' * 72}")
    t0 = time.time()
    results = walk_forward_e20(df, asia_window, asia_kwargs, cfg["London"], cfg["NY"])
    ...
    return {"label": label, "total_19q": total, "asia_pnl": float(asia_pnl),
            "asia_delta": float(asia_pnl - 151), "total_recent": recent_total,
...
def main():
    ...
    # e20a — Tighter SL (box_edge instead of extreme)
    summaries.append(run_variant("e20a · Asia tighter SL (box_edge)", df,
        asia_kwargs={"sl_mode": "box_edge", "tp_mode": "box_mid"}))

    # e20b — Opposite-edge TP (full reversion)
    summaries.append(run_variant("e20b · Asia opposite-edge TP (full reversion)", df,
        asia_kwargs={"sl_mode": "extreme", "tp_mode": "opposite_edge"}))

    # e20c — Non-adaptive max_attempts=3
    summaries.append(run_variant("e20c · Asia non-adaptive max_att=3", df,
        asia_kwargs={"sl_mode": "extreme", "tp_mode": "box_mid", "max_attempts_override": 3}))

    # e20d — Late-Asia window (21:00-23:00 ET = 1260-1380)
    summaries.append(run_variant("e20d · Asia late-window 21-23 ET", df,
        asia_window=(1260, 1380),
        asia_kwargs={"sl_mode": "extreme", "tp_mode": "box_mid"}))

    # e20e — COMBO: tighter SL + opposite-edge TP
    summaries.append(run_variant("e20e · Asia COMBO: tighter SL + opposite TP", df,
        asia_kwargs={"sl_mode": "box_edge", "tp_mode": "opposite_edge"}))

    out = ROOT / "data" / "phase7_e20_results.json"
    with open(out, 'w') as f:
        json.dump({'generated': datetime.datetime.now().isoformat(),
                   'e16b_baseline_asia': 151.0, 'target_asia': 300.0,
                   'variants': summaries}, f, indent=2)

    print("\n" + "═" * 72)
    print(" SUMMARY · e20 Asia R:R + Window")
    print("═" * 72)
