...
| 37 | Phase 43 Monkey Test (Robustness) | 2026-05-12 | 6 params swept, default rank #1-#4 | RESEARCH | INSIGHT | Parameter robustness check S.Y.S.T.E.M. method Step T. Sweep 6 params (retest_tol, sl_buffer, tp_mult, max_wait, be_trigger_r, max_attempt) ±20-40%. Default rank #1-#4 in all 6 sweeps. Worst trade stable -25 to -28pt across edge values. Verdict: SYSTEM ROBUST (no curve-fit cliff). Bonus finding: BE trigger 1.5R might beat 1.0R (queued Phase 44). |
| 38 | Phase 44 BE Trigger Sweep | 2026-05-12 | Full 5y engine, 7 variants 0.5R-2.0R | INSIGHT | TUNING | Sweep be_trigger_r dengan full engine (ATR + TP boost + NY delay). Result: BE 1.0R current = $213/yr → BE 1.5R = $518/yr (+143% boost) → BE 1.75R = $507. WR drop 48.8→40.8 but TP hits +27% (1291→1635). Worst trade SAMA -$28. Bonus finding hints regime split (Phase 46 queued). |
| 39 | Phase 45 S/R Strategy REJECTED | 2026-05-12 | 4 variants PDH/PDL/PP touch+reject + break+retest | RESEARCH | REJECTED | User wanted secondary strategy parallel to PT Box (S/R level-based, 24h window). 4 variants tested. ALL negative: V1 -$472/yr, V2 -$606, V3 +bias -$563, V4 break+retest -$104 (best, still negative). Reasons: WR 35-39% (low), spread eats edge, mean-revert math negative. 8× IRON LAW confirmed (e23/P16/e45/P21/P29/P39/P40/P45). PT Box mechanism = unique edge. Pine UNCHANGED. |
| 40 | Phase 46 Trump-Era Revalidation | 2026-05-12 | BE trigger sweep per era split | INSIGHT | REGIME | User question: backtest pre-Trump (Biden) relevant to current? Sweep BE trigger separately. RESULT: Optimal BE trigger SHIFT per era. Biden 2021-2024 best=1.5R (+$189/yr). Trump-2 2025-2026 best=1.75R (+$1,494/yr). Income scale Biden→Trump 7.9× (189→1494). Same lot 0.02, same system, beda regime. Recommend deploy BE 1.5R cross-era robust (wins both regimes, Biden #1 / Trump #2). |
| 41 | Phase 47 Mentor V3 REJECTED | 2026-05-12 | Mentor's 5min box + displacement + 3pt SL + 1:10 RR | RESEARCH | REJECTED | User shared mentor's SMC framework (Sweep+S/D+FVG+displacement). Test V3 style systematic: 5-min box + 3pt SL + 1:5/1:10/1:15 TP + displacement entry. ALL variants -$28 to -$60/yr. TP hits=0 across all variants. 1:10 RR never reached di gold M1. Reason: mentor edge = SUBJECTIVE HTF context (zone/OB/FVG quality reading), bukan mechanical displacement. 9× IRON LAW confirmed. Mentor system manual works for mentor (1000+ hr experience), systematic = no edge. User path forward: LEARN SMC manually (6-12 mo) + PT Box auto (mental hybrid). |
| 42 | SMC↔PT Box Reference + HTF Bias Widget | 2026-05-12 | Vault doc + dashboard page 12 | DELIVERABLE | EDUCATION | Mapping mentor's SMC framework to PT Box equivalents (Liquidity Pool=box range, Sweep=breakout, OB=box edge, Mitigation=pullback, etc). Vault doc `PT-Box-vs-SMC-Reference.md` for daily learning. Plus HTF Bias compute script + dashboard widget page 12: daily score 0-100 dari 6 factors (3-day trend, open vs PDC/PDM, 5-day range zone, prior day type, round number). Pure mental filter aid, no Pine code change. User uses pre-market 5-min ritual: open page → check today's bias → adjust PT Box signal eksekusi (full/half/skip per bias direction). |
| 43 | Phase 48-49 Trump Param + Small Box | 2026-05-12 | Statistical confidence + simplified engine sweep | RESEARCH | INSIGHT | Phase 48: Sample n=1907 trades Trump-only ADEQUATE, WR 95% CI [37.6-42%]. Daily income $4-8/day Trump pace. User goal $30-50/day = need 3.6-6× scale modal. Phase 49: simplified engine sweep box duration (5/10/15/30/60/90 min) × entry mode (BO/BO_strong/PB). 90min PB best $690 BUT invalid comparison (missing ATR + TP boost). Iron Law concerning small box. |
| 44 | Phase 50 Small Box FULL Engine | 2026-05-12 | Apples-to-apples ATR+TPboost+BE1.5R | RESEARCH | INSIGHT | Re-test with FULL engine. Trump-only results: 90min PB $1,254/yr (best) | 60min BO_strong $1,244 (close 2nd) | 30min BO $931 | 60min BO $939 | 90min BO_strong $775 | 60min PB $855 | 120min PB $764. Small box (5-15min) still negative. NY 30min BO DISASTER (-$524 MaxDD -$1,364). User insight valid: 60min BO_strong = viable alt to 90min PB. |
| 45 | Phase 51 Per-Session Distribution | 2026-05-12 | Per-session + distribution + pain metrics | INSIGHT | HYBRID DISCOVERY | User feedback CRITICAL: aggregate hide pain. Rebuilt analysis with per-session breakdown + daily PnL distribution P10/P25/P50/P75/P90 + pain metrics (worst day, MaxDD, consec loss). Pine v15.1 BE 1.5R per session: Asia $408/MaxDD $278/worst $75 | London $182/MaxDD $480/worst $67 | NY $664/MaxDD $243/worst $54 | TOTAL $1,254/MaxDD $480/worst $116 | days loss>$50 5.4% | days win>$50 12.9% | max consec 10. HYBRID COMBO discovery: Asia 90min PB + London **30min Breakout $1,085** + NY 90min PB = **$2,157/yr (4× current)**. Modal $200 lot 0.02 UNSAFE worst (-58% bal day, blow MaxDD). Recommend Stage 1: lot 0.01 + Pine v15.1 = $627/yr safer. User pain today -$29 = NORMAL P15-P25 day. |
